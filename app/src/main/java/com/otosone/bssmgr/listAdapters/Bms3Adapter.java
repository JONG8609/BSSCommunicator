...
public class Bms3Adapter extends BaseAdapter {

    private Context mContext;
    private List<JSONObject> mDataList;

    public Bms3Adapter(Context context, List<JSONObject> dataList) {
        mContext = context;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position == 0) { // For the title
            convertView = inflater.inflate(R.layout.title_list_bms3, parent, false); // Your custom layout for the title row.
        } else { // For the content
            if (convertView == null || (convertView.getTag() != null && convertView.getTag().equals("TITLE"))) {
                convertView = inflater.inflate(R.layout.status_list_bms3, parent, false);
            ...
            try {
                ...
                if (dataItem.has("cell_t") && !dataItem.isNull("cell_t")) {
                    JSONArray cellVArray = dataItem.getJSONArray("cell_t");
